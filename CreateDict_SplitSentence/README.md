# Important notice
This folder is a copy of the ["Measuring Corporate Culture Using Machine Learning" repository](https://github.com/MS20190155/Measuring-Corporate-Culture-Using-Machine-Learning) accompaning the same-titled paper by Li et al (2020). A few things are different:
1. `global_options.py` contains ESG seed words instead of Corporate Culture seed words, and the specifications are changed to the system used for this thesis.
2. `parse_parallel_split.py` and `preprocess_parallel_split.py` were added to use the CORENLP package to only split sentectes, which are then fed into FinBERT. The output is in `data/input/processed` with the name `documents_split.txt` and `document_ids_split.txt`.
3. The input are the QA sections from the earnings call, which are stored in `data/input/` with their IDs in `document_ids.txt`

The functions used in this thesis were:
 - `parse_parallel.py`
 - `parse_parallel_split.py`
 - `clean_and_train.py`
 - `create_dict.py`

Below is the original `README` by Li et al 2020
